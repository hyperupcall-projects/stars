...
from . import VERSION
from .githubrest import GitHubREST
...
@click.option('--topic_limit', default=500, show_default=True, type=int, help='ignored: REST API does not expose topic stargazer counts')
...
def starred(username, token, sort, topic, repository, filename, message, private, topic_limit):
    ...
    try:
        stars = GitHubREST(token).get_user_starred_by_username(username)
